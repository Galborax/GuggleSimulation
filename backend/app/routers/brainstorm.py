...
# Maps capital bracket → representative cash reserve in USD
_CAPITAL_TO_CASH: dict[str, float] = {
    "bootstrap": 5_000,
    "seed":      50_000,
    "series_a":  150_000,
}
...
async def chat(req: BrainstormChatRequest):
    ...
    sources_context, _ = await safe_fetch_sources(
...
async def generate_ideas(req: GenerateIdeasRequest):
    ...
    if session_id in _sessions:
        ...
        _sessions[session_id]["capital_bracket"] = req.capital_bracket
...
async def select_idea(req: SelectIdeaRequest):
    ...
    # Persist starting capital as cash_reserve so financial tools (Scenario Lab,
    # Runway Calculator, etc.) use the founder's actual available funds.
    capital_bracket = session.get("capital_bracket", "bootstrap")
    if "cash_reserve" not in session or session["cash_reserve"] is None:
        session["cash_reserve"] = _CAPITAL_TO_CASH.get(capital_bracket, 5_000)
