...
from app.services.market_service import get_sector_data, get_google_news
...
@router.get("/news")
async def google_news(q: str, limit: int = 10):
    query = q.strip()
    safe_limit = max(1, min(limit, 20))
    return await get_google_news(query=query, limit=safe_limit)
